...
func TestGetGradeA_MultipleScores(t *testing.T) {
	expected_value := "A"

	gradeCalculator := NewGradeCalculator()

	gradeCalculator.AddGrade("open source assignment", 99, Assignment)
...
func TestGetEmptyGrade(t *testing.T) {
	gradeCalculator := NewGradeCalculator()

	actual_value := gradeCalculator.GetFinalGrade()
	
	if actual_value != "F" {
		t.Errorf("Expected F , got '%s' instead", actual_value)
	}
}
...
func TestGetString(t *testing.T) {
	...
	if Assignment.String() == "" {
		t.Errorf("Must enter a non-empty string")
	}
